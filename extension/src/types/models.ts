...
export interface PerRequestRule {
  hostPattern: string;
  scheme?: string;
  proxy: any; // Firefox proxy info object
}
...
export interface OptionsMessage {
  type: 'getProfiles' | 'saveProfile' | 'deleteProfile' | 'getRules' | 'saveRule' | 'deleteRule' | 'importConfig' | 'exportConfig' | 'testRule' | 'getLogs' | 'clearLogs';
  data?: unknown;
}

export interface OptionsResponse {
  type: 'profiles' | 'rules' | 'config' | 'testResult' | 'logs' | 'success' | 'error';
  data?: unknown;
}